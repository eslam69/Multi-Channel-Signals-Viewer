...
class Ui_MainWindow(QMainWindow):
    def setupUi(self, MainWindow):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(
            QtGui.QPixmap("icons8-play-button-circled-96.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        ...
        icon1 = QtGui.QIcon()
        icon1.addPixmap(
            QtGui.QPixmap("icons8-pause-squared-96.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        ...
        icon2 = QtGui.QIcon()
        icon2.addPixmap(
            QtGui.QPixmap("icons8-erase-128xxx.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
        ...
        self.open_channels = [self.open_ch1, self.open_ch2, self.open_ch3, self.open_ch4, self.open_ch5]
        ...
        # creating timers
        # self.timers[0] = QtCore.QTimer()
        # self.timer2 = QtCore.QTimer()
        # self.timer3 = QtCore.QTimer()
        # self.timer4 = QtCore.QTimer()
        # self.timer5 = QtCore.QTimer()
        self.N_CHANNELS = 5
        self.timers = [QtCore.QTimer() for i in range(self.N_CHANNELS)]

        #data containers
        self.x = [0 for i in range(self.N_CHANNELS)]
        self.y = [0 for i in range(self.N_CHANNELS)]
        self.data = [0 for i in range(self.N_CHANNELS)]
        self.idx = [0 for i in range(self.N_CHANNELS)]
        self.timer_intervals = [100, 60, 20, 20, 20]
        self.graphicsViews = [ self.graphicsView, self.graphicsView_2, self.graphicsView_3, self.graphicsView_4, self.graphicsView_5]
        self.data_lines = [None for i in range(self.N_CHANNELS)]
        ...
        self.open_ch1.triggered.connect(lambda: self.load(0))
        self.open_ch2.triggered.connect(lambda: self.load(1))
        self.open_ch3.triggered.connect(lambda: self.load(2))
        self.open_ch4.triggered.connect(lambda: self.load(3))
        self.open_ch5.triggered.connect(lambda: self.load(4))
    ...
    def read_file(self,n):
        ...
        if pathlib.Path(path).suffix == ".txt":
            self.data[n] = np.genfromtxt(path, delimiter=",")
            self.x[n] = self.data[n][:, 0]
            self.y[n] = self.data[n][:, 1]
            self.x[n] = list(self.x[n][:])
            self.y[n] = list(self.y[n][:])
    ...
    def load(self,n):
        ...
        self.data_lines[n] = self.graphicsViews[n].plot(self.x[n], self.y[n], pen=self.pen)
        self.graphicsViews[n].plotItem.setLimits(xMin=0, xMax=12, yMin=-0.6, yMax=0.6)

        self.idx[n] = 0
        self.timers[n].setInterval(self.timer_intervals[n])

        self.timers[n].timeout.connect(lambda: self.update_plot_data(n))
        self.timers[n].start()
    def load1(self):
        self.read_file1()
        self.pen = pg.mkPen(color=(255, 0, 0))
        ...
        self.idx1 = 0
        self.timers[0].setInterval(100)

        self.timers[0].timeout.connect(self.update_plot_data1)
        self.timers[0].start()
    ...
    def update_plot_data(self,n):
        x = self.x[n][: self.idx[n]]
        y = self.y[n][: self.idx[n]]
        self.idx[n] += 10
        if self.idx[n] > len(self.x[n]):
            self.idx[n] = 0
        if self.x[n][self.idx[n]] > 0.5:
            self.graphicsViews[n].setLimits(
                xMin=min(x, default=0), xMax=max(x, default=0)
            )

        self.graphicsViews[n].plotItem.setXRange(
            max(x, default=0) - 0.5, max(x, default=0)
        )

        self.data_lines[n].setData(x, y)  # Update the data.
